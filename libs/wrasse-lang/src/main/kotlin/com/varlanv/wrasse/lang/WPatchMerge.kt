 ...
 * appended if it is new. Both operations return [existing] itself, same instance, when they would
 * change nothing, so a caller can compare identity to skip a rewrite.
 */
object WPatchMerge {
    ...
    fun remove(
        ...
        filePath: String,
    ): List<FileEdits> = if (existing.none { it.filePath == filePath }) {
        existing
    } else {
        existing.filterNot {
