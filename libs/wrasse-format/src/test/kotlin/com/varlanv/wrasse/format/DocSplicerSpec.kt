    ...
    BaseSpec(
        {
            ...
            should("replace a whole import directive with a multi-line atomic run (no-wildcard-imports expansion shape)") {
                val doc = Doc.Concat(listOf(Doc.Text("import a.*", 0, 10), Doc.Break(BreakKind.HARD, "\n", start = 10, end = 11)), 0, 11)
            ...
            should("bail (return null) when a cut lands strictly inside a Break's elided trailing-indent tail") {
                val doc = Doc.Concat(listOf(Doc.Break(BreakKind.HARD, "\n", start = 0, end = 5), Doc.Text("body()", 5, 11)), 0, 11)
            ...
            should(
                ...
                val innerIf = Doc
                    .Concat(listOf(Doc.Text("if (true)", 2, 11), Doc.Text(" ", 11, 12), Doc.Text("return \"yes\"", 12, 25)), 2, 25)
                val doc = Doc
                    .Concat(
                        listOf(
                            Doc.Text("{", 0, 1),
            ...
            should(
                "splice a non-zero-width close edit (a following branch) via the ordinary fullyCovered replace, no sibling insertion needed",
            ) {
                val content = Doc
                    .Concat(
                        listOf(
