    ...
    override fun initRule(config: WrasseRuleConfig): WNodeRule {
        ...
        return object : WNodeRule {
            ...
            override fun enterNode(ctx: WContext, reporter: WReporter): Boolean {
                ...
                val facts = CatchParameterText.parse(ctx.sourceText.subSequence(ctx.startOffset, ctx.endOffset)) ?: return false
                val message = TooGenericExceptionCaughtDecision.decide(facts.typeText, facts.name) ?: return false
                reporter.report(ruleId, message, ctx.startOffset + facts.nameStart, ctx.startOffset + facts.nameEnd, this)
                return false
            }
        }
    }
