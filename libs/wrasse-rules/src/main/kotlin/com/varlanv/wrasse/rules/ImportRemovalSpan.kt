 ...
 *
 * If the directive is the only non-whitespace content on its line(s) (checked against
 * [sourceText] between the enclosing line boundaries), the whole line is removed, including its
...
object ImportRemovalSpan {
    ...
    fun compute(sourceText: CharSequence, startOffset: Int, endOffset: Int): WEdit? {
        ...
        val newlineAtOrAfterEnd = indexOfNewlineFrom(sourceText, endOffset)
        val lineEnd = if (newlineAtOrAfterEnd >= 0) newlineAtOrAfterEnd else sourceText.length

        val prefixBlank = isBlank(sourceText, lineStart, startOffset)
        val suffixBlank = isBlank(sourceText, endOffset, lineEnd)

        if (!prefixBlank || !suffixBlank) return null

        val deleteEnd = if (newlineAtOrAfterEnd >= 0) newlineAtOrAfterEnd + 1 else lineEnd
        return WEdit(lineStart, deleteEnd, "")
    }

    private fun lineStartBefore(sourceText: CharSequence, offset: Int): Int {
        var i = offset - 1
        while (i >= 0 && sourceText[i] != '\n') i--
        return i + 1
    }

    private fun indexOfNewlineFrom(sourceText: CharSequence, offset: Int): Int {
        var i = offset
