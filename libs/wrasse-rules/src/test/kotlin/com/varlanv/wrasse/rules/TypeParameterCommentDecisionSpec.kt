...
class TypeParameterCommentDecisionSpec :
    BaseSpec(
        {

            should("report a comment inside a type parameter") {
                TypeParameterCommentDecision.decide(WNodeType.TYPE_PARAMETER, precededByNewline = false) shouldBe
            ...
            should("report a comment directly in a type parameter list not preceded by a newline") {
                TypeParameterCommentDecision.decide(WNodeType.TYPE_PARAMETER_LIST, precededByNewline = false) shouldBe
                    "A comment in a type parameter list is only allowed when placed on a separate line"
            }
