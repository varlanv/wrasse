...
private fun packageStars(vararg fqns: String): List<WResolvedImport> = fqns.map {
    WResolvedImport(fqn = it, isStarImport = true, resolvedParentClassFqName = null, resolved = true)
}
...
class UnusedStarDecisionSpec : BaseSpec({
    ...
    should(
        "report unused when a KDoc bracket reference's leading segment is covered by another star's own attribution",
    ) {
