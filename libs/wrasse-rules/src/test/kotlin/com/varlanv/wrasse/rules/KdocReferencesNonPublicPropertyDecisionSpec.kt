    ...
    BaseSpec(
        {
            ...
            should("suppress when a non-private function shares the property's name") {
                KdocReferencesNonPublicPropertyDecision.hasNonPrivateSameNameMember("peek", listOf("peek" to false)) shouldBe true
            }

            should("not suppress when the same-name function is itself private") {
                KdocReferencesNonPublicPropertyDecision.hasNonPrivateSameNameMember("peek", listOf("peek" to true)) shouldBe false
            }

            should("not suppress when no other member shares the property's name") {
                KdocReferencesNonPublicPropertyDecision.hasNonPrivateSameNameMember("prop1", listOf("other" to false)) shouldBe false
            }
