...
private fun record(text: String, startOffset: Int): ImportOrderingRecord =
ImportOrderingRecord(startOffset, startOffset + text.length, text)

class ImportOrderingDecisionSpec :
    BaseSpec(
        {
            ...
            should("strip backticks from the sort key so a quoted identifier sorts by its plain letters") {
                ...
                val records = listOf(record("import org.mockito.Mockito.`when`", 0), record("import org.mockito.Mockito.verify", 40))
