...
class UnnecessaryPartOfBinaryExpressionDecisionSpec : BaseSpec({
    ...
    should("report a duplicate operand") {
        UnnecessaryPartOfBinaryExpressionDecision.decide(listOf("foo", "bar", "foo")) shouldBe
            UnnecessaryPartOfBinaryExpressionDecision.MESSAGE
    }
    ...
    should("strip whitespace outside string and character literals only") {
        ...
        UnnecessaryPartOfBinaryExpressionDecision.normalizeOperand("\"buy back\" in lower") shouldBe
            "\"buy back\"inlower"
        UnnecessaryPartOfBinaryExpressionDecision.normalizeOperand("s.endsWith(\"x \\\" y\")") shouldBe
            "s.endsWith(\"x \\\" y\")"
    ...
    should("report when every operand is the same") {
        UnnecessaryPartOfBinaryExpressionDecision.decide(listOf("foo", "foo")) shouldBe
            UnnecessaryPartOfBinaryExpressionDecision.MESSAGE
    }
})
