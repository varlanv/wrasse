...
private fun star(
    ...
    startOffset: Int,
    endOffset: Int,
): StarImportRecord = StarImportRecord(packageFqName, startOffset, endOffset)

private fun topLevelCallable(
    packageFqName: String,
    name: String,
): WCallableUsage = WCallableUsage(packageFqName = packageFqName, classFqName = null, name = name)
...
private fun packageStar(
    fqn: String,
): List<WResolvedImport> = listOf(
    WResolvedImport(fqn = fqn, isStarImport = true, resolvedParentClassFqName = null, resolved = true),
)

private fun memberStar(
    fqn: String,
): List<WResolvedImport> = listOf(
    WResolvedImport(fqn = fqn, isStarImport = true, resolvedParentClassFqName = fqn, resolved = true),
)

private fun unresolvedStar(
    fqn: String,
): List<WResolvedImport> = listOf(
    WResolvedImport(fqn = fqn, isStarImport = true, resolvedParentClassFqName = null, resolved = false),
)
...
class WildcardExpansionDecisionSpec : BaseSpec({
    ...
    should(
        "NOT exclude a symbol covered only by an ALIASED explicit import of the same FQN, when its plain name is also written",
    ) {
        val edit = WildcardExpansionDecision.decide(
            star = star("p.aux", startOffset = 0, endOffset = 14),
            allStars = listOf(star("p.aux", startOffset = 0, endOffset = 14)),
    ...
    should(
        "bail with no edit when an attributed symbol's simple name is also used under a different FQN (conflict shape)",
    ) {
        val edit = WildcardExpansionDecision.decide(
    ...
    should(
        "bail with no edit when an attributed symbol's simple name collides with a default-resolved symbol (flip shape)",
    ) {
        val edit = WildcardExpansionDecision.decide(
    ...
    should(
        "bail with no edit when a cross-star collision puts the same simple name under two different attributed FQNs",
    ) {
    ...
    should(
        "not bail when a KDoc bracket reference's leading segment is covered only by a sibling star's own attribution",
    ) {
    ...
    should(
        "GATE: drop a member-callable attribution whose owner's simple name is never written (instance-member access)",
    ) {
        val edit = WildcardExpansionDecision.decide(
            star = star("p.aux", startOffset = 0, endOffset = 14),
            allStars = listOf(star("p.aux", startOffset = 0, endOffset = 14)),
