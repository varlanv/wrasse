...
class QualifiedUsageDecisionSpec : BaseSpec({
    ...
    should(
        "report an already-imported candidate even when its simple name collides with another used FQN's own simple name (kryptoid dual-Instant shape, detekt asymmetry)",
    ) {
        val sourceText = "val c = a.b.C"
        val usage = qualifier(sourceText, "a.b.C", "a.b.C", "a.b")
    ...
    should(
        "skip a same-package usage (no explicit import at all) when an unrelated explicit import shadows the same simple name",
    ) {
        val sourceText = "val c = a.b.C"
        val usage = qualifier(sourceText, "a.b.C", "a.b.C", "a.b")
    ...
    should(
        "not skip a same-package usage just because its own class declaration writes the same simple name elsewhere",
    ) {
    ...
    should(
        "not skip when the simple name's only other occurrence is a fully-qualified constructor call of the same target",
    ) {
    ...
    should(
        "attach a new-import request when no import exists, same package doesn't apply, and the package isn't a default import",
    ) {
        val sourceText = "val c = a.b.C"
        val usage = qualifier(sourceText, "a.b.C", "a.b.C", "a.b")
