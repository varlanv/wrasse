...
class SyncInAsyncDecisionSpec : BaseSpec({
    ...
    should("not report runBlocking with no governing async context") {
        SyncInAsyncDecision.decide(isRunBlockingTrailingLambdaCall = true, hasGoverningAsyncContext = false) shouldBe
            null
    }

    should("not report when the call is not the runBlocking trailing-lambda shape") {
        SyncInAsyncDecision.decide(isRunBlockingTrailingLambdaCall = false, hasGoverningAsyncContext = true) shouldBe
            null
    }
})
