...
class MppCommonCheckerDoubleFireSpec :
    BaseSpec(
        {

            val config = """{"rules":{"no-semicolons":{"level":"error"}}}"""
            val commonSource = TestSource("common/Common.kt", "package sample\n\nexpect fun platformName(): String\n\nval x = 1;\n")
            val platformSource = TestSource("platform/Platform.kt", "package sample\n\nactual fun platformName(): String = \"jvm\"\n")

            should("report the common source's violation exactly once in a legacy -Xcommon-sources MPP compile") {
                val harness = WrasseTestHarness(wrasseConfig = config, multiPlatformCommonSources = setOf(commonSource.path))
