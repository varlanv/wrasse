...
class IdempotenceCycleSpec : BaseSpec({
    ...
    should(
        "preserve multiplicity: two identical flag-only diagnostics both survive, two identical fixed ones both vanish",
    ) {
        val fixed = diagnostic("wrasse: fixable-rule: on line1", line = 1, column = 1, lineEnd = 1, columnEnd = 6)
        val flagOnly = diagnostic("wrasse: flag-only-rule: on line3", line = 3, column = 1, lineEnd = 3, columnEnd = 6)
        val survivors = IdempotenceCycle.expectedSurvivorKeys(
    ...
    should("pass silently when P1 was applied to every file") {
        IdempotenceCycle.assertPatchFullyApplied(
            listOf(FileApplyResult.Applied(Path.of("/tmp/sample/test.kt"), editCount = 2)),
        )
    }
