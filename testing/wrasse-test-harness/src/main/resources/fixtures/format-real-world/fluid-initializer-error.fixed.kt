...
class Desk(private val adaptersByExchange: Map<String, String>) {
    fun exchangesWithAdapter(request: List<String>): List<String> {
        val exchangesWithAdapter = request.filter { exchange ->
            adaptersByExchange.containsKey(exchange) && exchange.isNotEmpty()
        }
        return exchangesWithAdapter
    }

    fun aggregate(marketDataSourceWithLongName: List<String>): List<String> {
        val aggregatedCandlesticksByMinuteBucket =
            marketDataSourceWithLongName.mapCollectedIntoCandles { collectedItems ->
